...
A run may be continuing an earlier session: an `In progress` task was interrupted, so check the working tree before redoing that work. Otherwise start at the first sub-task that is not `Done`.
...
6. **Human review.** Present the sub-task, a summary of the diff, the verification result, and anything the sub-agent raised that you rejected. Wait.
...
8. **Close it out.** Mark the sub-task `Done` in `plan.md`.
...
- Step 8 sets `task.md`'s **Status** to `Done` and reports: if a task under the same two-letter code is still not `Done`, name the next one — `/clear`, then `/plan-lite {next task id}`.
